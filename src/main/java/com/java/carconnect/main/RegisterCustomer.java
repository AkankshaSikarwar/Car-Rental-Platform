		...
		try {
			...
			do {
				...
				if(isInvalidCustomer!=null) {
					...
					System.out.println(RED+"Customer Id already exists. Please enter "
							+ "a unique Customer Id."+RESET);
				}
				else {
					...
					do {
						try {
							isInvalidDate = false;
							System.out.print("Enter Registration Date (in yyyy-mm-dd form) : ");
							dateString = sc.next();
							...
							if(registrationDate.after(new Date())) {
								isInvalidDate = true;
								throw new InvalidInputException(RED+"Entered Date cannot be ahead of current "
										+ "Date."+RESET);
							}
...
//					customer.setRegistrationDate((java.sql.Date) registrationDate);
					java.sql.Date sqlRegistrationDate = new java.sql.Date(registrationDate.getTime());
                    customer.setRegistrationDate(sqlRegistrationDate);
